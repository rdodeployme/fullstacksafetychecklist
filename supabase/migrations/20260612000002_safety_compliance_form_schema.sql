-- Recycle Group / JUNK Safety Compliance schema
-- This migration defines the form-based safety compliance backend requested for:
-- profiles, vehicles, equipment, form templates, submissions, issues, updates,
-- and notifications.
...
do $$
begin
  create type public.safety_issue_status as enum (
    'open',
    'in_progress',
    'waiting_on_parts',
    'waiting_on_external',
...
do $$
begin
  create type public.safety_issue_priority as enum ('low', 'medium', 'high', 'critical');
exception
  when duplicate_object then null;
end $$;
...
create table if not exists public.form_submissions (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint form_submission_single_asset check (
    num_nonnulls(vehicle_id, equipment_id) <= 1
  )
);
...
create table if not exists public.issues (
  ...
  title text not null,
  description text,
  status public.safety_issue_status not null default 'open',
  priority public.safety_issue_priority not null default 'medium',
...
create table if not exists public.issue_updates (
  ...
  created_by uuid references public.profiles(id) on delete set null,
  status_from public.safety_issue_status,
  status_to public.safety_issue_status,
  note text not null,
  created_at timestamptz not null default now()
);
...
alter table public.issues
  ...
  add column if not exists answer_id uuid references public.form_answers(id) on delete set null,
...
create index if not exists form_templates_status_order_idx
  on public.form_templates(status, display_order);
create index if not exists form_templates_category_idx on public.form_templates(category);
create index if not exists form_sections_template_order_idx
  on public.form_sections(template_id, display_order);
create index if not exists form_questions_template_order_idx
  on public.form_questions(template_id, display_order);
create index if not exists form_questions_section_order_idx
  on public.form_questions(section_id, display_order);

create index if not exists form_submissions_template_idx
  on public.form_submissions(template_id);
create index if not exists form_submissions_submitted_by_idx
  on public.form_submissions(submitted_by);
create index if not exists form_submissions_vehicle_idx
  on public.form_submissions(vehicle_id);
create index if not exists form_submissions_equipment_idx
  on public.form_submissions(equipment_id);
create index if not exists form_submissions_submitted_at_idx
  on public.form_submissions(submitted_at desc);

create index if not exists form_answers_submission_idx
...
create index if not exists form_answers_failure_idx
  on public.form_answers(is_failure)
  where is_failure = true;

create index if not exists issues_status_priority_idx
  on public.issues(status, priority);
...
drop policy if exists "staff_read_active_vehicles" on public.vehicles;
create policy "staff_read_active_vehicles"
on public.vehicles for select
to authenticated
using (status <> 'retired' or public.is_supervisor_or_admin());

drop policy if exists "maintenance_manage_vehicles" on public.vehicles;
create policy "maintenance_manage_vehicles"
on public.vehicles for all
to authenticated
using (public.is_maintenance_or_manager())
with check (public.is_maintenance_or_manager());

drop policy if exists "staff_read_active_equipment" on public.equipment;
create policy "staff_read_active_equipment"
on public.equipment for select
to authenticated
using (status <> 'retired' or public.is_supervisor_or_admin());

drop policy if exists "maintenance_manage_equipment" on public.equipment;
create policy "maintenance_manage_equipment"
on public.equipment for all
to authenticated
using (public.is_maintenance_or_manager())
with check (public.is_maintenance_or_manager());

drop policy if exists "staff_read_active_templates" on public.form_templates;
create policy "staff_read_active_templates"
on public.form_templates for select
to authenticated
using (status = 'active' or public.is_supervisor_or_admin());

drop policy if exists "supervisors_manage_templates" on public.form_templates;
create policy "supervisors_manage_templates"
on public.form_templates for all
to authenticated
using (public.is_supervisor_or_admin())
with check (public.is_supervisor_or_admin());

drop policy if exists "staff_read_form_sections" on public.form_sections;
create policy "staff_read_form_sections"
...
drop policy if exists "supervisors_manage_form_sections" on public.form_sections;
create policy "supervisors_manage_form_sections"
on public.form_sections for all
to authenticated
using (public.is_supervisor_or_admin())
with check (public.is_supervisor_or_admin());

drop policy if exists "staff_read_form_questions" on public.form_questions;
create policy "staff_read_form_questions"
...
drop policy if exists "supervisors_manage_form_questions" on public.form_questions;
create policy "supervisors_manage_form_questions"
on public.form_questions for all
to authenticated
using (public.is_supervisor_or_admin())
with check (public.is_supervisor_or_admin());

drop policy if exists "staff_insert_own_submissions" on public.form_submissions;
create policy "staff_insert_own_submissions"
on public.form_submissions for insert
to authenticated
with check (submitted_by = auth.uid());

drop policy if exists "staff_read_own_submissions" on public.form_submissions;
create policy "staff_read_own_submissions"
on public.form_submissions for select
to authenticated
using (submitted_by = auth.uid() or public.is_supervisor_or_admin());

drop policy if exists "supervisors_update_submissions" on public.form_submissions;
create policy "supervisors_update_submissions"
on public.form_submissions for update
to authenticated
using (public.is_supervisor_or_admin())
with check (public.is_supervisor_or_admin());

drop policy if exists "staff_insert_answers_for_own_submission" on public.form_answers;
create policy "staff_insert_answers_for_own_submission"
...
drop policy if exists "staff_read_relevant_answers" on public.form_answers;
create policy "staff_read_relevant_answers"
...
using (
  exists (
    select 1 from public.form_submissions
    where form_submissions.id = form_answers.submission_id
      and (form_submissions.submitted_by = auth.uid() or public.is_supervisor_or_admin())
  )
);

drop policy if exists "staff_create_issues" on public.issues;
create policy "staff_create_issues"
on public.issues for insert
to authenticated
with check (created_by = auth.uid());

drop policy if exists "staff_read_relevant_issues" on public.issues;
create policy "staff_read_relevant_issues"
...
drop policy if exists "maintenance_update_issues" on public.issues;
create policy "maintenance_update_issues"
on public.issues for update
to authenticated
using (public.is_maintenance_or_manager())
with check (public.is_maintenance_or_manager());

drop policy if exists "staff_create_issue_updates" on public.issue_updates;
create policy "staff_create_issue_updates"
...
drop policy if exists "staff_read_relevant_issue_updates" on public.issue_updates;
create policy "staff_read_relevant_issue_updates"
...
drop policy if exists "system_or_admin_create_notifications" on public.notifications;
create policy "system_or_admin_create_notifications"
